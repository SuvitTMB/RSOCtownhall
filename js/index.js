...
var CheckFound = 0;
//var sStatusOrder = 0;
var sGroupRound = "RSOCTownhall2023#2";
//var sGroupRound = "NaNForum#1";
...
function Connect_DB() {
  ...
  dbRegMember = firebase.firestore().collection("BootMember");
  CheckData();
  //CheckRegister();
}
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      if(doc.data().statusconfirm==9) {
        ...
        //xEmpID = "80362";
        EidProfile = doc.id;
        sessionStorage.setItem("EmpID_RSOC", xEmpID);
        ...
        CheckRegister();
        //alert("Found:"+doc.data().statusconfirm);
        //CheckRewards();      
      }
    });
    if(CheckFoundData==0) {
...
function CheckRegister() {
  ...
  var xTime = "";
  var xCheck = 0;
  dbRegMember.where('EmpID','==', parseInt(sessionStorage.getItem("EmpID_RSOC")))
  .where('CampRound','==', sGroupRound)
  .limit(1)
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      xCheck = 1;
      xTime = doc.data().TimeIN;
    }); 
    document.getElementById('loading').style.display='none';
    if(xTime!="" && xCheck==1) {
      document.getElementById('OldSurvey').style.display='block';
    } else if(xTime=="" && xCheck==1) {
      document.getElementById('id01').style.display='block';
    } else {
      document.getElementById('id02').style.display='block';
    }
...
/*
  ...
  dbRegMember.where('EmpID','==',sessionStorage.getItem("LineID"))
  .where('CampRound','==', sStatusOrder)
...
function CheckRewards() {
  //console.log(sessionStorage.getItem("EmpID_RSOC"));
